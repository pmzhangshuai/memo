...
import (
	...
	_ "github.com/lib/pq" // 确保导入 PostgreSQL 驱动
...
`
	// 准备参数（PostgreSQL 使用 $1, $2, ... 作为参数占位符）
...
`
	...
	if err != nil {
		...
		// 	// 23505 是 PostgreSQL 中唯一约束冲突的错误码
...
func (d *DB) GetFollowingList(ctx context.Context, userID int32) ([]*store.User, error) {
	...
	// PostgreSQL uses $1, $2, ... for parameters, but for IN clause with multiple values,
	// we need to use a different approach like using ANY with an array.
	// However, to keep it simple and consistent with the original logic,
	// we'll create a placeholder string similar to other databases.
	placeholders := fmt.Sprintf("($%d", 1)
	for i := 2; i <= len(followingUserIDs); i++ {
	...
	// Create a slice of interface{} to hold the followingUserIDs for the query
...
WHERE following_user_id = $1`
	...
	// Create a placeholder string for the IN clause
	placeholders := fmt.Sprintf("($%d", 1)
	for i := 2; i <= len(followerUserIDs); i++ {
		placeholders += fmt.Sprintf(",$%d", i)
	}
	placeholders += ")"
...
WHERE id IN %s`, placeholders)
	...
	// Create a slice of interface{} to hold the followerUserIDs for the query
